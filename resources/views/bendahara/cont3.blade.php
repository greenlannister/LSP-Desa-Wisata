...
<aside class="sidenav navbar navbar-vertical navbar-expand-xs border-radius-lg fixed-start ms-2  bg-white my-2" id="sidenav-main">
  <div class="sidenav-header">
    ...
    <a class="navbar-brand px-4 py-3 m-0" href=" pnghttps://demos.creative-tim.com/material-dashboard/pages/dashboard " target="_blank">
    ...
    <div class="row">
      <div class="col-12">
        <div class="col-12 text-end">
          <button type="button" class="bg-gradient-dark mb-4 btn btn-primary" data-bs-toggle="modal" data-bs-target="#ModalObta">
              <span class="materspanal-symbols-rounded text-sm">+</span>&nbsp;&nbsp;Add New Object Wisata
          </button>
        </div>
        @if($errors->any())
        ...
        <div class="card my-4">
          <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
            <div class="bg-gradient-dark shadow-dark border-radius-lg pt-4 pb-3">
              <h6 class="text-white text-capitalize ps-3">Object WIsata table</h6>
            </div>
          </div>
              ...
              <table class="table align-items-center mb-0">
                <thead>
                  <tr>
                    ...
                    <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Kategori Wisata</th>
                ...
                <tbody>
                  ...
                  <tr>
                    <td>
                      <p class="text-xs font-weight-bold mb-0">{{ $objek->nama_wisata }}</p>
                    </td>
                    <td>
                      <p class="text-xs mb-0">{{ Str::limit($objek->deskripsi, 50) }}</p>
                    </td>
                    <td>
                      <p class="text-xs mb-0">{{ Str::limit($objek->fasilitas, 50) }}</p>
                    </td>
                    <td>
                      <p class="text-xs mb-0">{{ $objek->kategori_wisata->kategori_wisata }}</p>
                    </td>
                    <td>
                      @if($objek->foto1)
                        <img src="{{ asset('storage/' . $objek->foto1) }}" class="avatar avatar-sm me-3 border-radius-lg" alt="foto1">
                      @else
                    ...
                    <td>
                      @if($objek->foto2)
                        <img src="{{ asset('storage/' . $objek->foto2) }}" class="avatar avatar-sm me-3 border-radius-lg" alt="foto2">
                      @else
                    ...
                    <td>
                      @if($objek->foto3)
                        <img src="{{ asset('storage/' . $objek->foto3) }}" class="avatar avatar-sm me-3 border-radius-lg" alt="foto3">
                      @else
                    ...
                    <td class="align-middle">
                      <!-- Tombol Edit -->
                      <a href="#" data-bs-toggle="modal" data-bs-target="#ModalObtadit-{{ $objek->id }}" class="text-secondary font-weight-bold text-xs" data-toggle="tooltip" data-original-title="Edit homestay">
                        Edit
                      </a>
                    
                      <!-- Tombol Delete -->
                      <form id="delete-form-{{ $objek->id }}" action="{{ route('obta-wisata.destroy', $objek->id) }}" method="POST" style="display: inline;">
                        @csrf
                        @method('DELETE')
                        <a href="#" onclick="confirmDelete({{ $objek->id }})" class="text-danger font-weight-bold text-xs ms-2" data-toggle="tooltip" data-original-title="Hapus objek wisata">
                          Delete
                        </a>
                      </form>
                    </td>
...
@include('bendahara.object.edit')
@include('bendahara.object.create')
@endsection