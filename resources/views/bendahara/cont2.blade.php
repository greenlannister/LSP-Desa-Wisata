    ...
    <div class="row">
      <div class="col-12">
        <div class="col-12 text-end">
          <button type="button" class="bg-gradient-dark mb-4 btn btn-primary" data-bs-toggle="modal" data-bs-target="#ModalPaket">
              <span class="materspanal-symbols-rounded text-sm">+</span>&nbsp;&nbsp;Add New Package
          </button>
        </div>
        @if($errors->any())
        ...
        <div class="card my-4">
          <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
            <div class="bg-gradient-dark shadow-dark border-radius-lg pt-4 pb-3">
              <h6 class="text-white text-capitalize ps-3">Tabel Paket Wisata</h6>
            </div>
          </div>
              ...
              <table class="table align-items-center mb-0">
                <thead>
                  <tr>
                    ...
                    <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Harga / Pack</th>
                ...
                <tbody>
                  ...
                  <tr>
                    <td>
                      <p class="text-xs font-weight-bold mb-0">{{ $paket->nama_paket }}</p>
                    </td>
                    <td>
                      <p class="text-xs mb-0">Rp {{ number_format($paket->harga_per_pack, 0, ',', '.') }}</p>
                    </td>
                    <td>
                      <p class="text-xs mb-0">{{ Str::limit($paket->deskripsi, 50) }}</p>
                    </td>
                    <td>
                      <p class="text-xs mb-0">{{ Str::limit($paket->fasilitas, 50) }}</p>
                    </td>
                    <td>
                      @if($paket->foto1)
                        <img src="{{ asset('storage/' . $paket->foto1) }}" class="avatar avatar-sm me-3 border-radius-lg" alt="foto1">
                      @else
                    ...
                    <td>
                      @if($paket->foto2)
                        <img src="{{ asset('storage/' . $paket->foto2) }}" class="avatar avatar-sm me-3 border-radius-lg" alt="foto2">
                      @else
                    ...
                    <td>
                      @if($paket->foto3)
                        <img src="{{ asset('storage/' . $paket->foto3) }}" class="avatar avatar-sm me-3 border-radius-lg" alt="foto3">
                      @else
                    ...
                    <td class="align-middle">
                      <!-- Tombol Edit -->
                      <a href="#" data-bs-toggle="modal" data-bs-target="#ModalEditPaket-{{ $paket->id }}" class="text-secondary font-weight-bold text-xs" data-toggle="tooltip" data-original-title="Edit paket wisata">
                        Edit
                      </a>

                      <!-- Tombol Delete -->
                      <form id="delete-form-{{ $paket->id }}" action="{{ route('paket-wisata.destroy', $paket->id) }}" method="POST" style="display: inline;">
                        @csrf
                        @method('DELETE')
                        <a href="#" onclick="confirmDelete({{ $paket->id }})" class="text-danger font-weight-bold text-xs ms-2" data-toggle="tooltip" data-original-title="Hapus paket wisata">
                          Delete
                        </a>
                      </form>
                    </td>
...
@include('bendahara.package.edit')
@include('bendahara.package.create')
@endsection